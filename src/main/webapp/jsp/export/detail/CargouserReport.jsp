...
$(function() {
	...
	tableWin.loadSearch(eval(getval("selects")), {
		tableId : 'myTable',
		formId : 'myForme',
		dataExport:{url:'cargouserreport/exportData'  
			//,onclick:'data_export(\''+property.dataExport.url + '\',\''+ property.formId + '\')
			}
		});
	tableWin.loadSearchValue([
	          				{filed : "business_type",data : [{id :"null",name : "不限"}, {name : "即时性用车"},{name : "拼车"},{name : "计4划性用车"}]},
	          				{filed : "truck_audit_status",data : [{id :"null",name : "不限"}, {name : "已审核通过"},{name : "未审核通"},{name : "未审核"}]},
	          				{filed : "instantaneity_category",data : [{id :"null",name : "不限"}, {name : "专业市场"},{name : "物流"}]},
	          				{filed : "truck_device_type",data : [{id :"null",name : "不限"}, {name : "专业市场"},{name : "物流"}]},
	          				{filed : "coupon_status",data : [{id :"null",name : "不限"}, {name : "初始状态"},{name : "刚生成"},{name : "正在使用"},{name : "已使用"},{name : "已过期"},{name : "已删除"}]}
	          			]);
	$("#search_"+"cargo_register_time"+"_str").val('2016-01-01 00:00');
	$("#search_"+"cargo_register_time"+"_end").val('2016-01-01 02:00');
	//初始化table
	myTable = tableWin.loadTable('myTable', 'cargouserreport/query', eval(getval("fileds")), {
