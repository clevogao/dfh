...
<script type="text/javascript">
	var myModal;
	var myTable;
	/* tableWin.search=function(){
		var data={};
		$.each($("#myForme").serializeArray(),function(i,o) {   
			eval("data."+o.name+" = "+"'"+o.value+"'");
		});
		ajaxFunctionAsyncTrue("timelycoupon/count", data,function(count){
			alert(count);
		});
	} */
	$(function() {
		...
		tableWin.loadSearch(select, {
			tableId : 'myTable',
			formId : 'myForme',
			dataExport:{url:'timelycoupon/exportData'
				//,onclick:'data_export(\''+property.dataExport.url + '\',\''+ property.formId + '\')
				}
			});
		$("#search_"+"coupon_create_time_str").val(newDate());
		$("#search_"+"coupon_create_time_end").val(newDate(1));
		//初始化table
		myTable = tableWin.loadTable('myTable', 'timelycoupon/query', eval(getval("fileds")), {
