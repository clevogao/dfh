...
public class ReflectUtil {
	...
	public static Object geObjectUtilFromMap(Class<?> clazz,
		Map<String, Object> mapField) throws Exception, ParseException {
		Object obj = null;
			...
			for (Field field : fields) {
				...
				if (mapField.containsKey(field.getName())) {
					...
					//Map 类型
						...
						Map mapObject = ObjectUtil.cast(newInstance(fieldType));
						mapObject.putAll((Map) ObjectUtil.cast(value));
						value = mapObject;
					}
					//Collection 类型
					else if(isImpByInterface(fieldType,Collection.class)){
						Collection lisObjectUtil = ObjectUtil.cast(newInstance(fieldType));
						lisObjectUtil.addAll((Collection) ObjectUtil.cast(value));
						value = lisObjectUtil;
					}
					//如果没有匹配到类型且参数是 Map 类型,就判定是复杂类型,递归调用方法
					else if(isImpByInterface(value.getClass(),Map.class)){
						value = geObjectUtilFromMap(fieldType, (Map<String, Object>) ObjectUtil.cast(value));
					}
	...
	public static Map<String, Object> getMapfromObject(Object obj) throws Exception{
		...
		Map<String, Object> fieldValues =  ReflectUtil.getFieldValues(obj);
		//如果是 java 标准类型
